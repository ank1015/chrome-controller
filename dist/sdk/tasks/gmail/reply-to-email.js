...
const GMAIL_REPLY_BODY_SELECTOR = 'div[aria-label="Message Body"][role="textbox"], [role="textbox"][aria-label="Message Body"]';
...
export async function replyToEmail(options) {
    ...
    return await withWebBrowser(async (browser) => {
        ...
        try {
            ...
            const replyVisible = initialSnapshot.isReplyVisible || (await openReplyComposer(tab, initialSnapshot.subject));
            if (!replyVisible) {
                return buildReplyResult({
...
export function normalizeReplyAttachmentPaths(value) {
    if (value === undefined) {
        return [];
    }
...
export function previewReplyBody(value, maxLength = 220) {
...
export function parseReplyToEmailCliArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg.startsWith('--attachment=')) {
            attachmentPaths = [...attachmentPaths, arg.slice('--attachment='.length).trim()].filter((path) => path.length > 0);
            continue;
        }
        if (arg.startsWith('--attach=')) {
            attachmentPaths = [...attachmentPaths, arg.slice('--attach='.length).trim()].filter((path) => path.length > 0);
            continue;
        }
        if (arg === '--send') {
            send = true;
...
async function openReplyComposer(tab, subject) {
...
async function readReplySnapshot(tab) {
    await waitForGoogleHumanVerificationIfNeeded(tab, {
...
async function applyReplyBody(tab, desiredBody) {
    return await tab.evaluate(`(() => {
      const bodyCandidates = Array.from(document.querySelectorAll(${JSON.stringify(GMAIL_REPLY_BODY_SELECTOR)}));
      const bodyElement = bodyCandidates.find((element) => {
        if (!(element instanceof HTMLElement)) {
...
async function uploadReplyAttachments(tab, attachmentPaths) {
    ...
    const predicateSource = attachmentNames
        .map((fileName) => `Array.from(document.querySelectorAll('[download_url], .dL, .vI, .aV3, [aria-label*="Attachment"]')).some((element) => {
    ...
    await tab.waitForIdle(1_500);
    const snapshot = await readReplySnapshot(tab);
    return attachmentNames.filter((fileName) => snapshot.attachmentNames.some((attachmentName) => attachmentName.includes(fileName)));
}
async function clickSendAndWait(tab) {
...
function buildReplyResult({ status, requestedUrl, snapshot, resolvedOptions, attachedFileNames, message, }) {
    return {
        status,
        requestedUrl,
        page: snapshot.page,
...
function resolveReplyOptions(options) {
...
async function saveReplyToEmailResultToTemp(result) {
...
function renderReplyToEmailMarkdown(result, outputPath) {
    const lines = [
        '# Gmail reply',
        '',
...
function createReplyToEmailUsage() {
    return `Usage:
  npx @ank1015/llm-extension gmail reply-to-email [options]
...
Examples:
  npx @ank1015/llm-extension gmail reply-to-email --url "https://mail.google.com/mail/u/0/#inbox/19d09010e9d7747f" --body "Thanks for the update."
  npx @ank1015/llm-extension gmail reply-to-email --url "https://mail.google.com/mail/u/0/#inbox/19d09010e9d7747f" --body "See attached." --attachment ./notes.txt --send`;
}
export async function runReplyToEmailCli(argv = process.argv.slice(2)) {
...
//# sourceMappingURL=reply-to-email.js.map