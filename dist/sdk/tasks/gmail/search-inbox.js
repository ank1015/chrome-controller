...
import { buildGmailThreadOpenUrl } from './fetch-n-mails.js';
...
export async function searchInbox(options) {
    ...
    const snapshot = await withWebBrowser(async (browser) => {
        ...
        try {
            ...
            for (let pageIndex = 0; pageIndex < pagesToScan; pageIndex += 1) {
                appendUniqueMails(collectedMails, seenMailKeys, pageSnapshot.mails, resolvedOptions.count);
                if (collectedMails.length >= resolvedOptions.count) {
                    break;
                }
                if (pageSnapshot.noResults ||
                ...
                const movedToNextPage = await goToNextSearchResultsPage(tab, pageSnapshot.firstMailSignature);
                if (!movedToNextPage) {
...
export function parseSearchInboxCliArgs(argv) {
    if (argv.length === 0 || argv.includes('--help') || argv.includes('-h')) {
        throw new Error(createSearchInboxUsage());
    }
    let query = '';
...
export function buildInboxSearchQuery(query) {
...
export function buildGmailInboxSearchUrl(query) {
...
async function waitForSearchResultsReady(tab) {
...
async function readSearchPageSnapshot(tab) {
    await waitForGoogleHumanVerificationIfNeeded(tab, {
        readyPredicate: GMAIL_SEARCH_RESULTS_READY_PREDICATE,
        label: 'Gmail verification',
    });
...
async function goToNextSearchResultsPage(tab, previousFirstMailSignature) {
...
function appendUniqueMails(target, seenMailKeys, pageMails, count) {
...
function getMailDedupeKey(mail) {
    return [
        mail.legacyThreadId || '',
        mail.threadId || '',
        mail.sender || '',
...
function resolveOptions(options) {
    ...
    return {
        query,
        count: typeof options.count === 'number' && Number.isFinite(options.count) && options.count > 0
...
function encodeGmailSearchHashValue(value) {
    return encodeURIComponent(value).replace(/%20/g, '+');
}
async function saveSearchInboxResultToTemp(result) {
...
function renderSearchInboxMarkdown(result, outputPath) {
    const lines = [
        '# Gmail search inbox',
        '',
    ...
    for (const mail of result.mails) {
        lines.push(`${mail.index + 1}. **${mail.sender || 'Unknown sender'}** — ${mail.subject || '(no subject)'}`);
...
export async function runSearchInboxCli(argv = process.argv.slice(2)) {
...
function createSearchInboxUsage() {
    return `Usage:
  npx @ank1015/llm-extension gmail search-inbox --query <words> [options]

Options:
  --query, -q <words>   Gmail search words
  --count, -n <number>  Number of matching mails to collect
  --no-launch           Do not try to launch Chrome automatically

Examples:
  npx @ank1015/llm-extension gmail search-inbox --query "digitalocean" --count 5
  npx @ank1015/llm-extension gmail search-inbox --query "invoices"`;
}
function formatMailState(mail) {
