...
export async function getEmail(options) {
    const resolvedOptions = resolveOptions(options);
    const { snapshot, downloadedAttachments, attachmentDownloadErrors } = await withWebBrowser(async (browser) => {
        ...
        try {
            ...
            const downloadResult = resolvedOptions.downloadAttachmentsToPath &&
                !snapshot.isLoginGate &&
                snapshot.messages.some((message) => message.attachments.length > 0)
                ? await downloadThreadAttachments(browser, tab, snapshot, resolvedOptions.downloadAttachmentsToPath)
...
export function normalizeGmailThreadUrl(url) {
...
export function normalizeAttachmentDownloadPath(path) {
...
export function normalizeGmailAttachmentDownloadUrl(downloadUrl) {
    ...
    const secondColonIndex = firstColonIndex >= 0 ? trimmedValue.indexOf(':', firstColonIndex + 1) : -1;
    const rawUrl = secondColonIndex >= 0 ? trimmedValue.slice(secondColonIndex + 1) : trimmedValue;
    return rawUrl.replace('https://mail.google.com/mail/u/0/https://mail.google.com/mail/u/0?', 'https://mail.google.com/mail/u/0?');
}
export function parseGetEmailCliArgs(argv) {
    if (argv.length === 0 || argv.includes('--help') || argv.includes('-h')) {
        throw new Error(createGetEmailUsage());
    }
    let url = '';
    let launch = true;
    let downloadAttachmentsToPath = null;
...
export function buildThreadContentText(messages) {
    const sections = messages
        .filter((message) => message.bodyText)
        .map((message, index) => {
        const lines = [`Message ${index + 1}`];
        if (message.from.name || message.from.email) {
            lines.push(`From: ${message.from.name || 'Unknown sender'}${message.from.email ? ` <${message.from.email}>` : ''}`);
        }
        if (message.toText) {
...
async function saveGetEmailResultToTemp(result) {
...
function renderGetEmailMarkdown(result, outputPath) {
    const lines = [
        '# Gmail email',
        '',
    ...
    for (const message of result.messages) {
        ...
        lines.push(`${message.index + 1}. **${sender}**${message.from.email ? ` <${message.from.email}>` : ''}`);
        if (message.timeText) {
            lines.push(`   Time: ${message.timeText}`);
        }
        if (message.toText) {
...
function createGetEmailUsage() {
    return `Usage:
  npx @ank1015/llm-extension gmail get-email --url <gmail-thread-url> [options]
...
Examples:
  npx @ank1015/llm-extension gmail get-email --url "https://mail.google.com/mail/u/0/#inbox/19d09010e9d7747f"
  npx @ank1015/llm-extension gmail get-email --url "https://mail.google.com/mail/u/0/#inbox/..." --download-attachments-to ./downloads`;
}
async function waitForThreadReady(tab) {
...
async function expandTrimmedContent(tab) {
    await tab.evaluate(`(() => {
...
async function readThreadSnapshot(tab) {
    await waitForGoogleHumanVerificationIfNeeded(tab, {
        readyPredicate: GMAIL_THREAD_READY_PREDICATE,
        label: 'Gmail verification',
    });
...
async function downloadThreadAttachments(browser, tab, snapshot, targetDir) {
    await mkdir(targetDir, { recursive: true });
    const downloadedAttachments = [];
    const attachmentDownloadErrors = [];
    for (const message of snapshot.messages) {
        for (const attachment of message.attachments) {
            try {
                const beforeDownloads = await browser.listDownloads();
                const beforeIds = new Set(beforeDownloads
                ...
                const download = attachment.downloadUrl
                    ...
                    : await downloadAttachmentViaPageClick(browser, tab, message.index, attachment.index, beforeIds, attachment.name);
                ...
                attachmentDownloadErrors.push(error instanceof Error
                    ? error.message
...
async function triggerAttachmentDownload(tab, messageIndex, attachmentIndex) {
...
async function downloadAttachmentViaChrome(browser, rawDownloadUrl) {
    const normalizedDownloadUrl = normalizeGmailAttachmentDownloadUrl(rawDownloadUrl);
    const downloadId = await browser.chrome('downloads.download', {
        url: normalizedDownloadUrl,
    });
    if (typeof downloadId !== 'number') {
        throw new Error('Chrome did not return a download ID for the Gmail attachment.');
    }
    return await browser.waitForDownload({ id: downloadId }, {
        requireComplete: true,
        timeoutMs: DOWNLOAD_WAIT_TIMEOUT_MS,
    });
}
async function downloadAttachmentViaPageClick(browser, tab, messageIndex, attachmentIndex, previousDownloadIds, attachmentName) {
...
async function waitForFreshDownload(browser, previousDownloadIds) {
    const deadline = Date.now() + DOWNLOAD_WAIT_TIMEOUT_MS;
    while (Date.now() < deadline) {
        const downloads = await browser.listDownloads();
        const freshDownload = downloads.find((download) => download.state === 'complete' &&
    ...
    throw new Error(`Timed out waiting for attachment download within ${DOWNLOAD_WAIT_TIMEOUT_MS}ms.`);
}
async function moveDownloadedFile(sourcePath, targetDir, attachmentName) {
    const safeName = sanitizeAttachmentFileName(attachmentName || basename(sourcePath) || 'attachment');
...
async function getAvailableTargetPath(targetDir, fileName) {
...
export async function runGetEmailCli(argv = process.argv.slice(2)) {
